...
export const AnimatedTestimonials = ({
  ...
  // initialize rotations to zeros so server and client HTML match on first render
  const [rotations, setRotations] = useState(() => testimonials.map(() => 0));

  // compute random rotations only on the client after mount to avoid hydration mismatch
  useEffect(() => {
    const computed = testimonials.map(() => Math.floor(Math.random() * 21) - 10);
    setRotations(computed);
  ...
  const randomRotateY = (i) => rotations[i] || 0;
  return (
    <div
